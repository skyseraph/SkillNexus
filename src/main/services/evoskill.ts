...
import { EvoSkillEngine } from './sdk/evoskill-engine'
...
export async function runEvoSkill(config: { skillId: string; maxIterations?: number }): Promise<EvoSkillResult> {
  const db = getDb()
  const engine = new EvoSkillEngine(
