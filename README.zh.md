...
[English](README.md) · [博客](https://skyseraph.github.io)· [专栏](https://skyseraph.github.io/series/skill-nexus/)
...
**Skill** 是遵循 [Agent Skills 开放标准](https://agentskills.io) 的模块化能力单元：一个包含 `SKILL.md`（Markdown + YAML frontmatter）的目录。Skill 安装到 `~/.claude/skills/`，可被 Claude Code、Cursor、Windsurf、Gemini CLI 等 AI 工具自动发现和调用。
...
`npm install` 会自动触发 `postinstall`，提取适合当前平台和 Electron 版本的 `better-sqlite3` 原生二进制，无需单独执行 rebuild。
...
如果代理导致 `npm install` 静默跳过 `better-sqlite3`，应用仍可正常启动——`postinstall` 会自动复制内置的 JS wrapper，并在运行时根据实际 Electron 版本（通过 `process.versions.modules`）选择正确的预编译二进制。

如果遇到 `NODE_MODULE_VERSION` 不匹配错误，说明 `prebuilds/` 中缺少对应版本的预编译包，详见 [doc/windows_install.md](doc/windows_install.md)。
...
SkillNexus 博文系列，共 10 篇，从工具使用到技术细节。
